...
void convert_json()
{
  // Take json parameters from a digitizer
  auto link = std::string("dig1://caen.internal/usb?link_num=0");
  uint64_t handle;
  auto err = CAEN_FELib_Open(link.c_str(), &handle);
  auto jsonSize = CAEN_FELib_GetDeviceTree(handle, nullptr, 0) + 1;
  char *json = new char[jsonSize];
  CAEN_FELib_GetDeviceTree(handle, json, jsonSize);
  std::string jsonStr = json;
  err = CAEN_FELib_Close(handle);
